...
function makeCfg(ratio: number): ClodPagesConfig {
  return {
    page: { chunks_per_page: 4, chunk_size: 16, halo_chunks: 1, quadtree_levels: 4 },
    simplify: {
      target_ratio_per_level: ratio,
      abandon_ratio: 0.85,
      target_error: 0.01,
...
for (const ratio of [0.5, 0.85, 0.98]) {
  try {
    const t0 = performance.now();
    buildWorld(16, 16, makeCfg(ratio));
    console.log(`ratio=${ratio} OK ${(performance.now() - t0).toFixed(0)}ms`);
  } catch (e) {
    console.error(`ratio=${ratio} FAIL`, (e as Error).message);
  }
}
