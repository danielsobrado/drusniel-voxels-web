...
for (const levels of [3, 4]) {
  const cfg = { ...base, page: { ...base.page, quadtree_levels: levels } };
  try {
    const t0 = performance.now();
    buildWorld(16, 16, cfg);
    console.log(`levels=${levels} OK ${(performance.now() - t0).toFixed(0)}ms`);
  } catch (e) {
    console.error(`levels=${levels} FAIL`, (e as Error).message);
  }
}
