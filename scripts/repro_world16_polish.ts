...
function makeCfg(polish: boolean): ClodPagesConfig {
  return {
    ...
    polish: { diagonal_flip: { ...DEFAULT_DIAGONAL_FLIP_CONFIG, enabled: polish } },
    selection: {
...
for (const polish of [true, false]) {
  try {
    ...
    buildWorld(16, 16, makeCfg(polish));
    console.log(`polish=${polish} OK ${(performance.now() - t0).toFixed(0)}ms`);
  } catch (e) {
    console.error(`polish=${polish} FAIL`, (e as Error).message);
  }
}
