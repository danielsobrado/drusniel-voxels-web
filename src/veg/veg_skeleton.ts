 ...
 * (gravity/light), wander noise, and cantilever droop; children spawn along the
    ...
    if (count > 0) {
      ...
      for (let gi = 0; gi < groups; gi++) {
        ...
        for (let wi = 0; wi < inWhorl; wi++) {
          ...
          const asymK =
            1 + sp.asym * (cDir.x * inst.biasX + cDir.z * inst.biasZ) * (isTrunk ? 1 : 0.4);
...
export function growSkeleton(
  ...
  const height =
    (sp.height[0] + rng.float() * (sp.height[1] - sp.height[0])) * (0.72 + instance.age * 0.36);
  const ctx: GrowCtx = {
    sp,
    rng,
    inst: instance,
